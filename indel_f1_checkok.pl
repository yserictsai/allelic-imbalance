	...
	@p1a_indel_array = ();
		...
		if( $#p1a_indel_array >= 0 ){
			...
			$p1a_indel_freq = $p1a_count{$p1a_indel_seq[0]}/$p1a_cov;
                        $p1a_basecall = $p1a_indel_seq[0] if( $p1a_indel_freq > 0.9 );
	...
	@p1b_indel_array = ();
        ...
        if( $p1b_cov >= 5 ){
			...
			$p1b_indel_freq = $p1b_count{$p1b_indel_seq[0]}/$p1b_cov;
                        $p1b_basecall = $p1b_indel_seq[0] if( $p1b_indel_freq > 0.9 );
                }
        }

#	print "$ref_name\t$ref_pos\t$p1a_basecall\t$p1b_basecall\n" if ( $p1a_basecall ne 'X' && $p1a_basecall ne 'X') ;
...
#	print "$ref_name\t$ref_pos\t$p1_mixbasecall\n" if ( defined $p1_mixbasecall);
	if( $p1_mixbasecall ne 'X' ){
	        print "$ref_name\t$ref_pos\t$p1_mixbasecall\n";
	}
